...
class WinnerController extends Controller
{
    public function win2d3d4d5d6d(){
        $winOrders = BillOrder::orderBy('id','desc')->where('status_win',1)->where('type','2d3d4d5d6d')->get();
    ...
    public function win340(){
        $winOrders = BillOrder::orderBy('id','desc')->where('status_win',1)->where('type','3/40')->get();
